...
void Path::reset(int droneId, Vector2D dronePossition) {
	owner = droneId;
	objectiveValue = 0;
	currentGoal = 1;
	allreadyHasAComNode = false;
	
	PathNode pathNodeSelf;
	pathNodeSelf.possition = dronePossition;
	pathNodeSelf.priority = 0;
	pathNodeSelf.iceberg = PATH_NO_ICEBERG;
	nodes.clear();
	nodes.push_back(pathNodeSelf);
}
...
bool Path::finished(Vector2D dronePos) {
	return (currentGoal >= nodes.size() 
		|| previousNode().isComNode 
		|| previousNode().getsCom);
	//TODO dette funker ikke, idet avstanden er unde drone FOW så er vi alt på neste
	//vi må istedet teste om forrige er bra


	//Nb tester ute å stoppe ved noder som trenger kom, blir dust å stoppe og returnere 
	//ved hver eneste node utafor
}
...
//timepoint is given relative to the movement speed of the drone and timestamp,
//we move one unit distance at one unit time
Vector2D locationAtTimepoin(Path* path, double timepoint, int* lastNode) {
	...
	*lastNode = 1;
	auto jt = path->nodes.begin() + 1;
	for (auto it = path->nodes.begin(); jt != path->nodes.end(); ++it) {
		distance = distance2D(it->possition, jt->possition);
		if (length + distance < timepoint) {
			length += distance;
		}
		else {
			//find the point timepoint - length from it to jt
			//how much we are missing to be at the timepoint, in the direction from it to jt
			Vector2D vectorToGoal = normalize((jt->possition - it->possition))*(timepoint - length);
			location = it->possition + vectorToGoal;
			return location;
		}
		jt++;
		(*lastNode)++;
	}
	return path->nodes.back().possition;
}